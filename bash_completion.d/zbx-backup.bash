...
# Place it to /etc/bash_completion.d/ and run:
# [server@user ~]$ . /etc/bash_completion.d/zbx-backup.bash
...
zbx_backup_autocomplete() {
	...
	MAIN_OPTS="--save-to --backup-with --temp-folder --compress-with --rotation --use-xtrabackup --use-mysqldump --db-only --db-user --db-password --db-name --add-to-backup --my-login-path --debug --force --help --version"
